...
/**
 * Locates a product page for an EAN on one of `sites` via Serper (a
 * third-party proxy over real Google search results, https://serper.dev)
 ...
 * paying for (and waiting on) a separate Serper call per retailer.
 *
...
export async function findProductUrlViaSerper(
  ...
  try {
    const res = await fetch("https://google.serper.dev/search", {
      ...
      body: JSON.stringify({ q: `${siteFilter(sites)} ${ean}` }),
